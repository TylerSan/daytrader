#!/usr/bin/env bash
# scripts/run_intraday_4h_2_launchd.sh — Phase 5.5 T12 (2026-05-05)
#
# launchd wrapper for intraday-4h-2 cadence (11:00 PT Mon-Fri).
...
echo "[run_intraday_4h_2_launchd] PATH=$PATH"
echo "[run_intraday_4h_2_launchd] PWD=$PROJECT_ROOT"
...
echo "[run_intraday_4h_2_launchd] preflight ok, invoking reports run --type intraday-4h-2"
uv run daytrader reports run --type intraday-4h-2 --no-pdf
rc=$?
echo "[run_intraday_4h_2_launchd] reports run exit=$rc"
echo "[run_intraday_4h_2_launchd] end $(date -Iseconds)"
exit "$rc"
